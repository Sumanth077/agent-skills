...
Usage:
    ...
    # Export to file
    uv run scripts/run_actor.py --actor ACTOR_ID --input '{}' --output influencers.csv --format csv

    # Export basic fields only
    uv run scripts/run_actor.py --actor ACTOR_ID --input '{}' --output influencers.csv --format csv --fields basic
"""
...
USER_AGENT = "apify-agent-skills/apify-influencer-discovery-1.0.0"
...
Examples:
  ...
  uv run scripts/run_actor.py \\
    ...
    --input '{"usernames": ["influencer1", "influencer2"]}'

  # Export all data to CSV
  uv run scripts/run_actor.py \\
    --actor "apify/instagram-hashtag-scraper" \\
    --input '{"hashtags": ["fitness"], "resultsLimit": 100}' \\
    --output influencers.csv --format csv

  # Export basic fields only
  uv run scripts/run_actor.py \\
    --actor "clockworks/tiktok-scraper" \\
    --input '{"profiles": ["@creator1"]}' \\
    --output tiktok-influencers.csv --format csv --fields basic
        """,
    )
    parser.add_argument(
        "--actor",
        required=True,
        help="Actor ID (e.g., apify/instagram-profile-scraper)",
    )
    parser.add_argument(
        "--input",
        required=True,
