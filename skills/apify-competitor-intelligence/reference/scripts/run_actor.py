...
Usage:
    ...
    # Export to file
    uv run scripts/run_actor.py --actor ACTOR_ID --input '{}' --output analysis.csv --format csv

    # Export basic fields only
    uv run scripts/run_actor.py --actor ACTOR_ID --input '{}' --output analysis.csv --format csv --fields basic
"""
...
ESSENTIAL_FIELDS = {
    ...
    "compass~Google-Maps-Reviews-Scraper": ["name", "url", "text", "stars", "publishedAtDate", "likesCount", "responseFromOwnerText"],
    # Booking.com
    "voyager~booking-scraper": ["name", "url", "address", "rating", "reviewScore", "price", "facilities", "description"],
    "voyager~booking-reviews-scraper": ["title", "url", "text", "rating", "date", "travelerType", "positiveText", "negativeText"],
    # Facebook
    ...
    "apify~facebook-followers-following-scraper": ["profileName", "profileUrl", "followersCount", "friendsCount", "isVerified"],
    "apify~facebook-reviews-scraper": ["text", "rating", "date", "reviewerName", "reviewerUrl", "pageName"],
    ...
    "streamers~youtube-comments-scraper": ["text", "author", "authorUrl", "likesCount", "publishedAt", "replyCount"],
    ...
    "clockworks~tiktok-video-scraper": ["webVideoUrl", "authorMeta.name", "text", "playCount", "diggCount", "commentCount", "shareCount", "createTime"],
...
Examples:
  ...
  uv run scripts/run_actor.py \\
    --actor "compass/crawler-google-places" \\
    --input '{"searchStringsArray": ["coffee shops"], "locationQuery": "Seattle, USA"}' \\
    --output analysis.csv --format csv

  # Export basic fields only
  uv run scripts/run_actor.py \\
    --actor "compass/crawler-google-places" \\
    --input '{"searchStringsArray": ["coffee shops"], "locationQuery": "Seattle, USA"}' \\
    --output analysis.csv --format csv --fields basic
        """,
    )
    parser.add_argument(
        "--actor",
